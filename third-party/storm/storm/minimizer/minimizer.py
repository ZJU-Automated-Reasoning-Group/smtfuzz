...
class minimize(object):
    def __init__(self, dir_path, file_path, solverbin, maxDepth, maxAssert, SEED, parsedArguments, iteration):
        ...
        print(colored("Incremental Mode = ", "white", "on_blue", attrs=["bold"]) + colored(str(self.incremental),
                                                                                           "yellow", attrs=["bold"]))
        print(colored("check_sat_using = ", "white", "on_blue", attrs=["bold"]) + colored(str(self.check_sat_using),
                                                                                          "yellow", attrs=["bold"]))
        print(colored("Theory = ", "white", "on_blue", attrs=["bold"]) + colored(str(self.theory), "yellow",
                                                                                 attrs=["bold"]))
        print(colored("Solver = ", "white", "on_blue", attrs=["bold"]) + colored(str(self.solver), "yellow",
                                                                                 attrs=["bold"]))
        self.min_depth = None
        ...
        file.writelines("\n" + str(iteration) + "," + str(SEED) + "," + str(max_depth) + "," + str(self.min_assert) +
                        "," + str(self.min_lines) + "," + str(self.min_bytes) + "," + str(
    ...
    def generate_and_run_mutants(self, maxDepth, maxAssert):
        ...
        signal = generate_mutants(smt_Object=smt_Object, path_to_directory=self.temp_dir, maxDepth=maxDepth,
                                  maxAssert=maxAssert,
    ...
    def minimizeLines(self, maxDepth, maxAssert):
        ...
        for i in range(10):
            print(colored("min LINE NUMBERS = " + str(self.get_number_of_lines(self.minimized_file_path)), "magenta",
                          attrs=["bold"]))
            ...
            if success_flag and minimum_number_of_lines > self.get_number_of_lines(mutant):
                print(
                    colored("new min LINE NUMBERS = " + str(self.get_number_of_lines(mutant)), "green", attrs=["bold"]))
    ...
    def minimizeDepth(self, minDepth, maxDepth):
        print(colored(
            "DEPTH BOUNDS: minDepth= " + str(minDepth) + ",  maxDepth= " + str(maxDepth) + "   maxAssert= " + str(
                self.min_assert), "white", "on_blue", attrs=["bold"]))
    ...
    def minimizeAssertions(self, minAssert, maxAssert):
        print(colored("ASSERT BOUNDS: maxDepth=" + str(self.min_depth) + ",   minAssert= " + str(
            minAssert) + ",  maxAssert= " + str(maxAssert), "white", "on_blue", attrs=["bold"]))
