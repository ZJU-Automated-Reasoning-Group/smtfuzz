...
class PreAnalyzer(object):
    def __init__(self, seed, ast, var_dict, model, logic, data, solver):
        ...
        self.real_op = ["tanh", "cosh", "sinh", "arctan2", "atan2", "arctan", "atan", "arccos", "acos", "arcsin",
                        "asin", "tan", "cos",
                        "sin", "log", "exp", "to_real", "/", "csc", "sec", "cot", "arccsc", "arcsec", "arccot",
                        "real.pi"]
        ...
        self.string_op = ["str.in_re", "str.<=", "str.<", "str.prefixof", "str.suffixof", "str.contains",
                          "str.is_digit",
                          "str.++", "str.at", "str.substr", "str.replace_all", "str.replace_re_all", "str.replace_re",
                          "str.replace", "str.from_code", "str.from_int", "str.len", "str.to_code", "str.to_int",
        ...
        self.return_re_op = ["str.to_re", "re.none", "re.all", "re.allchar", "re.++", "re.union", "re.inter", "re.*",
                             "re.comp",
    ...
    def pre_analysis(self):
        ...
        seed_info = {"var": self.var, "const": self.const, "term": self.terms, "evaluation": self.evaluations,
    ...
    def get_constraint(self, term, depth, p_node="root-node"):
        ...
        if p_node == "root-node":
            self.constraints[node] = [is_true(cur_eval), cur_ty, cur_eval,
                                      True]  # constraint, type, evaluation, possibility for mutation
        else:
            ...
            cond = self.sat_preserving_constraint(parent_op, parent_cond, cur_eval, cur_ty, idx, sub_evaluations,
                                                  sub_api_terms)
    ...
    def sat_preserving_constraint(self, parent_op, parent_cond, cur_eval, cur_type, node_idx, sub_evaluations,
                                  sub_api_terms):
        try:
            cond = cur_eval
            if parent_op in self.logic_op and parent_op != "ite":
                ...
                cond = get_compare_constraint(parent_op, parent_cond, cur_eval, cur_type, node_idx, sub_evaluations,
                                              sub_api_terms, self.solver, self.model)
            elif parent_op in self.real_op:
                cond = get_real_constraint(parent_op, parent_cond, cur_eval, cur_type, node_idx, sub_evaluations,
                                           sub_api_terms, self.solver, self.model)
            elif parent_op in self.int_op:
                cond = get_int_constraint(parent_op, parent_cond, cur_eval, cur_type, node_idx, sub_evaluations,
                                          sub_api_terms, self.solver, self.model)
            elif parent_op in self.arith_op:
                cond = get_arith_constraint(parent_op, parent_cond, cur_eval, cur_type, node_idx, sub_evaluations,
                                            sub_api_terms, self.solver, self.model)
            elif parent_op in self.string_op:
                cond = get_string_constraint(parent_op, parent_cond, cur_eval, cur_type, node_idx, sub_evaluations,
                                             sub_api_terms, self.solver, self.model)
            elif parent_op in self.reg_op:
                cond = get_regp_constraint(parent_op, parent_cond, cur_eval, cur_type, node_idx, sub_evaluations,
                                           sub_api_terms, self.solver, self.model)
            return cond
