...
from parameters import get_supported_theories
from parameters import get_parameters_dict
...
def record_soundness(home_directory, seed_file_path, buggy_mutant_path, seed, mutant_number, seed_theory,
                     fuzzing_parameters, option=None, cmd=None):
    ...
    error_logs += "\n"
    error_logs += "The command that was used to run octopus: " + cmd + "\n"

    create_file(error_logs, os.path.join(path_to_bug_dir, "error_logs.txt"))


def record_crash(home_directory, seed_file_path, buggy_mutant_path, seed, mutant_number, seed_theory,
                 fuzzing_parameters, option=None, cmd=None):
    temp_dir = os.path.join(home_directory, "temp")
    print(colored("Creating a crash folder at: ", "magenta", attrs=["bold"]) + temp_dir)
    print(colored("seed file path: ", "magenta", attrs=["bold"]) + seed_file_path)
    print(colored("buggy mutant file path: ", "magenta", attrs=["bold"]) + buggy_mutant_path)
