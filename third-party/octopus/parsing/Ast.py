import copy
from parsing.Types import ALL_OPS, ALL_OP_DICT, OPERATOR_OPS, CONNECTIVE_OPS, QUANTIFIER_OPS
...
class Script:
    def __init__(self, commands, global_vars):
        ...
        self.op_occs = []
        self.assert_cmd = []
        self.term_op_statis = dict()
        self.term_connect_statis = dict()
        self.term_q_statis = dict()
        self.depth = dict()
    ...
    def _count_ops(self, e):
        ...
        if isinstance(e, Term):
            for op in OPERATOR_OPS:
                op_list.append(str(e).count("(" + op + " "))
            for c in CONNECTIVE_OPS:
                connective_list.append(str(e).count("(" + c + " "))
            for quantifier in QUANTIFIER_OPS:
                quantifier_list.append(str(e).count("(" + quantifier + " "))
        self.term_op_statis[str(e)] = op_list
        self.term_connect_statis[str(e)] = connective_list
        self.term_q_statis[str(e)] = quantifier_list

    def _prefix_free_vars(self, prefix, e):
        """
        add prefix and collect statistics of each term
        including count of operators, connectives, quantifiers, and depth
        """
        if isinstance(e, str):
            self.depth.setdefault(e, 0)
            return
        if e.is_const:
            self.depth.setdefault(str(e), 0)
            return
        if e.is_var and e.type:
        ...
        if e.var_binders:
            for i, var in enumerate(e.var_binders):
                ...
                current_depth = self.depth.get(str(e.let_terms[i]), 0)
                if current_depth > self.depth.get(str(e), 0):
                    self.depth[str(e)] = current_depth
                self.depth[str(var)] = self.depth.get(str(e.let_terms[i]), 0)
                self._count_ops(e.let_terms[i])
                self.term_op_statis[str(var)] = self.term_op_statis.get(str(e.let_terms[i]), 0)
                self.term_connect_statis[str(var)] = self.term_connect_statis.get(str(e.let_terms[i]), 0)
                self.term_q_statis[str(var)] = self.term_q_statis.get(str(e.let_terms[i]), 0)
        else:
            self._count_ops(e)

        for s in e.subterms:
            self._prefix_free_vars(prefix, s)
            current_depth = self.depth.get(str(s), 0) + 1
            if current_depth > self.depth.get(str(e), 0):
                self.depth[str(e)] = current_depth
    ...
    def prefix_vars(self, prefix):
        ...
        for cmd in self.commands:
            if isinstance(cmd, DeclareConst):
                if "|" not in cmd.symbol:
                    cmd.symbol = prefix + cmd.symbol
                else:
                    cmd.symbol = "|" + prefix + cmd.symbol.replace("|", "") + "|"
            if isinstance(cmd, DeclareFun):
                if cmd.input_sort == "":
                    if "|" not in cmd.symbol:
                        cmd.symbol = prefix + cmd.symbol
        ...
        for global_var in self.global_vars:
            if "|" not in global_var:
                new_global_vars[prefix + global_var] = self.global_vars[global_var]
            else:
                new_global_vars["|" + prefix + global_var.replace("|", "") + "|"] = self.global_vars[global_var]
...
class Term:
    ...
    def __str__(self):
        ...
        if self.quantifier:
            ...
            s = (
                    "("
                    + str(self.quantified_vars[0][0])
                    + " "
                    + str(self.quantified_vars[1][0])
                    + ")"
            )
            if len(self.quantified_vars[0]) > 1:
                for i in range(1, n_vars):
                    s += (
                            " ("
                            + str(self.quantified_vars[0][i])
                            + " "
                            + str(self.quantified_vars[1][i])
                            + ")"
                    )
